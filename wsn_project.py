...
mpl.rcParams['axes.unicode_minus'] = False # Solve the problem that the negative sign '-' is displayed as a square when saving an image
...
class Leach(object):
    ...
    def cluster_head_selection():
        ...
        heads   = Leach.heads = [] # The list of cluster heads, each round is initialized to be empty
...
#                    print(temp_rand)
                    ...
                    if temp_rand <= Tn:
                        nodes[i].type = "CH" # This node is the cluster head of the current cycle
                        nodes[i].G = 1  # G is set to 1, this cycle can no longer be selected as a cluster head or (1/p)-1
        ...
        print("The number of CHs is:", len(heads), (WSN.n - WSN.n_dead), ("Energy of nodes-> "), nodes[i].energy)
    ...
    def cluster_formation():
        ...
        for member in members:
            ...
            min_dis = np.sqrt(WSN.xm ** 2 +  WSN.ym ** 2) # The broadcast radius within the cluster head node area
            head_id = None
            ...
            cluster[str(head_id)].append(member.id) # Add to the corresponding cluster head of the out-cluster class
    ...
    def steady_state_phase():
        ...
        for key, values in cluster.items():
            ...
            if n_member == 0: # If there are no cluster members, only the cluster head collects its own information and sends it to the base station
                ...
                new_data = WSN.DM * (n_member + 1) # Plus the data collected by the cluster head itself, the new data package after fusion
